...
robj *rdbLoadObject(int rdbtype, rio *rdb) {
    ...
    if (rdbtype == REDIS_RDB_TYPE_STRING) {
        ...
        while(isetlen--) {
            ...
            if ((ele = rdbLoadEncodedStringObject(rdb)) == NULL) return NULL;
            ele = tryObjectEncoding(ele);
            if (rdbLoadDoubleValue(rdb,&(scores[0])) == -1) return NULL;
            if (rdbLoadDoubleValue(rdb,&(scores[1])) == -1) return NULL;
            ...
            anode = avlInsert(tree, scores[0], scores[1], ele);
            dictAdd(tree->dict,ele,&anode->scores);
